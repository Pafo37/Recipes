...
public class LatestMealApiService {
    ...
    private void cancelLatestMealDownload() {
    ...
    private class LatestMealTask extends AsyncTask<String, Integer, RecipeModel> {

        @Override
        protected void onPreExecute() {
            if (recipesCallback != null) {
                NetworkInfo networkInfo = recipesCallback.getActiveNetworkInfo();
                if (networkInfo == null || !networkInfo.isConnected() ||
                        (networkInfo.getType() != ConnectivityManager.TYPE_WIFI
                                && networkInfo.getType() != ConnectivityManager.TYPE_MOBILE)) {
                    recipesCallback.showLatestMealResult(null);
                }
            }
        }
        ...
        protected void onPostExecute(RecipeModel recipeModel) {
            if (recipeModel != null && recipesCallback != null) {
                recipesCallback.showLatestMealResult(recipeModel);
            }
        }
    }

    private RecipeModel latestMealConnection(URL url) throws IOException {
    ...
    private RecipeModel readLatestMealStream(InputStream inputStream) throws IOException {
        JsonReader reader = new JsonReader(new InputStreamReader(inputStream, "UTF-8"));
        return readLatestRecipeData(reader);
    }

    private RecipeModel readLatestRecipeData(JsonReader reader) throws IOException {
    ...
    private RecipeModel readLatestRecipeFields(JsonReader reader) throws IOException {
        RecipeModel recipeModel = new RecipeModel();
        reader.beginArray();
        reader.beginObject();
        while (reader.hasNext()) {
